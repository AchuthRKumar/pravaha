...
const checkAndProcessAnnouncements = async () => {
    ...
    for (const announcement of latestAnnouncements.slice(0, 20).reverse()) {
        try {
            const { sm_id: nseId, sm_symbol_dtl: symbol, sm_name: companyName, an_dt: announcementTime, attchmnt_dtl: pdfPath } = announcement;
            const existingAnalysis = await Analysis.findOne({ nse_id: nseId });

            if (existingAnalysis) {
                console.log(`- Skipping already processed announcement for ${symbol}`);
                continue;
            }
            ...
            if (!extractedText) {
                console.warn(`⚠️ Could not extract text for announcement ID ${nseId}. Skipping.`);
                continue;
            }

            const analysisResult = await aiService.analyzeNews(extractedText);
            if (!analysisResult) {
                console.warn(`⚠️ AI analysis failed for announcement ID ${nseId}. Skipping.`);
                continue;
            }
            ...
            const newAnalysis = new Analysis({
                ...
                announcement_time: new Date(announcementTime),
                source_pdf_url: pdfPath, // pdfPath is now the full URL
                ...analysisResult
            });
